...
export default function TutorAuth() {
  ...
  function switchMode(m) { setMode(m); setError(''); setSuccess(''); setShowPassword(false) }
  ...
  async function handleSignup(e) {
    ...
    try {
      ...
      if (data.user) await supabase.from('profiles').upsert({ id:data.user.id, full_name:form.fullName, role:'tutor', email:form.email, school:form.school, phone:form.phone })
      ...
      <div style={s.wrapper}>
        <div style={s.panel} className="card card-elevated">
          <div style={s.header}>
            ...
            <h2 style={s.title}>{mode==='forgot'?'Reset Password':mode==='login'?'Tutor Sign In':'Tutor Sign Up'}</h2>
            <p style={s.subtitle}>{mode==='forgot'?"We'll send a reset link to your email.":mode==='login'?'Access your student roster and performance data.':'Create your tutor account to track student progress.'}</p>
          </div>
          ...
          {mode === 'login' && (
            <form onSubmit={handleLogin}>
              <div className="form-group"><label>Email Address</label><input type="email" placeholder="tutor@email.com" value={form.email} onChange={e => set('email', e.target.value)} required /></div>
          ...
          {mode === 'forgot' && (
            <form onSubmit={handleForgotPassword}>
              <div className="form-group"><label>Your Email Address</label><input type="email" placeholder="tutor@email.com" value={form.email} onChange={e => set('email', e.target.value)} required /></div>
          ...
          {mode === 'signup' && (
            <form onSubmit={handleSignup}>
              <div className="form-group"><label>Full Name</label><input type="text" placeholder="e.g. Thabo Sechele" value={form.fullName} onChange={e => set('fullName', e.target.value)} required /></div>
              <div className="form-group"><label>School or Organisation (optional)</label><input type="text" placeholder="e.g. Gaborone Primary School" value={form.school} onChange={e => set('school', e.target.value)} /></div>
              <div className="form-group"><label>Phone Number (optional)</label><input type="tel" placeholder="e.g. +267 71 234 567" value={form.phone} onChange={e => set('phone', e.target.value)} /></div>
              <div className="form-group"><label>Email Address</label><input type="email" placeholder="tutor@email.com" value={form.email} onChange={e => set('email', e.target.value)} required /></div>
              <div className="form-group"><label>Password</label><PasswordField {...pwProps} placeholder="At least 8 characters" minLength={8} /></div>
              <button type="submit" className="btn btn-primary btn-full mt-2" disabled={loading}>{loading?'Creating account…':'Create Tutor Account →'}</button>
            </form>
          )}
