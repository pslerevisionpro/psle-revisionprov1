...
function PasswordField({ placeholder, minLength, showPassword, onToggle, value, onChange }) {
  return (
    <div style={{ position:'relative' }}>
      <input type={showPassword?'text':'password'} placeholder={placeholder||'Your password'} value={value} onChange={onChange} required minLength={minLength} style={{ paddingRight:44 }}/>
      <button type="button" onClick={onToggle} style={eyeBtn} tabIndex={-1}>{showPassword?'🙈':'👁️'}</button>
    </div>
  )
}
...
export default function ParentAuth() {
  ...
  async function handleSignup(e) {
    ...
    try {
      ...
      if (data.user) await supabase.from('profiles').upsert({ id:data.user.id, full_name:form.fullName, role:'parent', email:form.email, child_name:form.childName, consent_privacy:true, consent_terms:true, consent_data:true, consent_date:new Date().toISOString() })
      ...
      <div style={s.wrapper}>
        <div style={s.panel} className="card card-elevated">
          <div style={s.header}>
            ...
            <h2 style={s.title}>{mode==='forgot'?'Reset Password':mode==='login'?'Parent Sign In':step===1?'Parent Sign Up':'Agreements & Consent'}</h2>
            <p style={s.subtitle}>{mode==='forgot'?"We'll send a reset link to your email.":mode==='login'?"Monitor your child's PSLE preparation.":step===1?'Create your parent account.':'Please read and accept the following.'}</p>
          </div>
          ...
          {mode === 'login' && (
            <form onSubmit={handleLogin}>
              <div className="form-group"><label>Email Address</label><input type="email" placeholder="parent@email.com" value={form.email} onChange={e => set('email', e.target.value)} required /></div>
          ...
          {mode === 'forgot' && (
            <form onSubmit={handleForgotPassword}>
              <div className="form-group"><label>Your Email Address</label><input type="email" placeholder="parent@email.com" value={form.email} onChange={e => set('email', e.target.value)} required /></div>
          ...
          {mode === 'signup' && step === 1 && (
            <form onSubmit={e => { e.preventDefault(); setStep(2) }}>
              <div className="form-group"><label>Your Full Name</label><input type="text" placeholder="e.g. Goitsemang Molefe" value={form.fullName} onChange={e => set('fullName', e.target.value)} required /></div>
              <div className="form-group"><label>Child's Name (optional)</label><input type="text" placeholder="e.g. Keabetswe Molefe" value={form.childName} onChange={e => set('childName', e.target.value)} /></div>
              <div className="form-group"><label>Email Address</label><input type="email" placeholder="parent@email.com" value={form.email} onChange={e => set('email', e.target.value)} required /></div>
          ...
          {mode === 'signup' && step === 2 && (
            <form onSubmit={handleSignup}>
              <div style={s.consentBox}>
                ...
                <p style={s.consentDesc}>Under the Botswana Data Protection Act 2018, we need your explicit consent before creating an account for your child.</p>
                {[
              ...
              <div style={{ display:'flex', gap:10, marginTop:16 }}>
                ...
                <button type="submit" className="btn btn-primary btn-full" disabled={loading || !allConsented}>{loading?'Creating account…':'Create Account →'}</button>
              </div>
            </form>
          )}
