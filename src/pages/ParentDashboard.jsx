      ...
      <div style={s.banner}>
        <div className="content-wrapper" style={s.bannerInner}>
          <div style={{ flex:1 }}>
            ...
            <p style={s.bannerSub}>
              {children.length === 0
                ? 'Link your child\'s account below to start monitoring their progress.'
          ...
          {selected && childProfile && (
            <div style={s.statRow}>
              {[
                ...
                { val:weeklyAttempts||'—', label:'This Week' },
        ...
        <div style={s.grid}>
          ...
          <div style={{ minWidth:0 }}>
            ...
            {children.length === 0 && !loading && (
              <div style={s.emptyState}>
                ...
                <h3 style={{ fontFamily:'var(--font-display)', fontSize:'1.4rem', color:'var(--forest)', marginBottom:8 }}>
                  No children linked yet
                </h3>
                <p style={{ color:'var(--charcoal-lt)', fontSize:'0.9rem', marginBottom:0 }}>
                  Use the form on the right to link your child's student account.
                </p>
              </div>
            )}
              ...
              <>
                ...
                <div style={s.summaryRow}>
                  <div style={s.summaryCard}>
                    <p style={s.summaryLabel}>Student</p>
                    <p style={s.summaryVal}>{childProfile.full_name}</p>
                  </div>
                  <div style={s.summaryCard}>
                    <p style={s.summaryLabel}>Standard</p>
                    <p style={s.summaryVal}>{childProfile.grade === 'std7' ? 'Standard 7' : 'Standard 6'}</p>
                  </div>
                  <div style={s.summaryCard}>
                    <p style={s.summaryLabel}>Overall Grade</p>
                    <p style={{ ...s.summaryVal, color: overallAvg>0?gradeColor(overallAvg):'#AAA' }}>
                      {overallAvg>0 ? `Grade ${gradeLetter(overallAvg)} (${overallAvg}%)` : 'No attempts yet'}
                    </p>
                  </div>
                  <div style={s.summaryCard}>
                    <p style={s.summaryLabel}>Activity This Week</p>
                    <p style={{ ...s.summaryVal, color: weeklyAttempts>0?gradeColor(70):'#AAA' }}>
                      {weeklyAttempts > 0 ? `${weeklyAttempts} quiz${weeklyAttempts>1?'zes':''}` : 'No activity'}
                    </p>
                  </div>
                </div>
                ...
                <div style={s.chartCard}>
                  <div style={s.chartHeader}>
                    <div>
                      ...
                      <p style={s.chartSub}>Last {Math.min(childResults.length,10)} attempts</p>
                    </div>
                    {overallAvg>0 && (
                  ...
                  {SUBJECTS.map(sub => {
                    ...
                    return (
                      <div key={sub.key} style={s.subCard}>
                        <Ring pct={best??0} size={52}/>
                        <div style={{ flex:1 }}>
                          <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:3 }}>
                            <span style={s.subEmoji}>{sub.emoji}</span>
                            <span style={s.subName}>{sub.name}</span>
                          </div>
                          {best !== null ? (
                            <p style={{ fontSize:'0.72rem', fontWeight:700, color:gradeColor(best) }}>
                              Best {best}% · Grade {gradeLetter(best)} · {subAttempts.length} attempt{subAttempts.length>1?'s':''}
                            </p>
                          ) : (
                            <p style={{ fontSize:'0.72rem', color:'#AAA', fontStyle:'italic' }}>Not attempted</p>
                          )}
                        </div>
                      </div>
                    )
                ...
                <div style={{ ...s.chartCard, marginTop:24 }}>
                  ...
                  {recentResults.length === 0 ? (
                    <p style={{ color:'#AAA', fontSize:'0.85rem', padding:'16px 0' }}>No quiz attempts yet.</p>
          ...
          <div>
            ...
            <div style={s.sideCard}>
              ...
              <p style={s.sideCardDesc}>Enter your child's student account email to link their progress to your dashboard.</p>
              <form onSubmit={linkChild} style={{ marginTop:14 }}>
                <div className="form-group">
                  <label>Child's Email</label>
                  <input
                    type="email" placeholder="child@email.com"
            ...
            <div style={s.consentCard}>
              ...
              <p style={s.consentText}>
                ...
                To withdraw consent or request data deletion, contact us at privacy@pslerevisionpro.co.bw
              </p>
            </div>

            {/* Parent tips */}
            <div style={s.tipCard}>
              <p style={s.tipBadge}>💡 Parent Tip</p>
              <p style={s.tipText}>
                {totalAttempts === 0
                  ? 'Encourage your child to take their first Science quiz today — it only takes 5 minutes.'
                  : weeklyAttempts === 0
                  ? 'No activity this week. A gentle reminder to revise can make a big difference.'
                  : overallAvg >= 70
                  ? 'Your child is performing well! Encourage them to try other subjects too.'
                  : 'Regular short revision sessions are more effective than occasional long ones.'}
              </p>
            </div>
...
const s = {
  ...
  childTab:     { display:'flex', alignItems:'center', gap:6, padding:'8px 16px', borderRadius:100, border:'1.5px solid var(--ivory-dk)', background:'var(--white)', cursor:'pointer', fontSize:'0.85rem', fontWeight:600, color:'var(--charcoal-lt)', fontFamily:'var(--font-body)' },
  ...
  summaryRow:   { display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))', gap:12, marginBottom:24 },
  summaryCard:  { background:'var(--white)', border:'1px solid var(--ivory-dk)', borderRadius:12, padding:'16px' },
  summaryLabel: { fontSize:'0.7rem', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.06em', color:'#AAA', marginBottom:6 },
  summaryVal:   { fontSize:'0.95rem', fontWeight:700, color:'var(--charcoal)' },
