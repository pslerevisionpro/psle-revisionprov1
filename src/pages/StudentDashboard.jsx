      ...
      <div className="content-wrapper" style={{ paddingTop:32, paddingBottom:64 }}>
        <div style={s.grid}>
          <div style={{ minWidth:0 }}>
            ...
            {/* Per-subject score history */}
            {!loading && [...new Set(allResults.map(r => r.subject).filter(Boolean))].length > 0 && (
              <div style={{ marginBottom:24 }}>
                ...
                {[...new Set(allResults.map(r => r.subject).filter(Boolean))].map(subject => (
                  <SubjectHistory key={subject} subjectName={subject} results={allResults.filter(r => r.subject === subject)} />
                ))}
              </div>
            )}
